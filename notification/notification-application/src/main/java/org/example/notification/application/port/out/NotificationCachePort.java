...
/**
 * Notification 정보 1차 캐시(Redis) 포트.
 *
 * <p>알림 정보는 <b>불변</b>(생성 후 상태 변화 없음)이라 stale 이 존재할 수 없다. 따라서 PER·SWR·분산락 없이
...
public interface NotificationCachePort {
    ...
    /**
     * 주어진 id 들의 알림 정보를 캐시에서 조회한다. 존재하는 항목만 반환한다(불변이라 stale 판정 불필요).
     * 캐시 장애 시 fail-open(빈 맵) 으로 동작한다.
     */
    Map<String, Notification> findByIds(Set<String> ids);

    /**
     * 알림 정보를 캐시에 적재(warm-up)한다. 생성 시 선적재와 조회 miss 시 lazy 적재 양쪽에서 쓴다.
     */
    void warmUpAll(List<Notification> notifications);

    /**
     * 알림 캐시를 무효화한다(soft-delete 등, 현재 호출부 없음).
     */
