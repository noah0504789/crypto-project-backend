...
- **Consumer 멱등 경계 유지**: `PriceAlertNotificationCommandService.create`의 `@Transactional("transactionManager")`이 `InboxEventService.save`의 `(consumer_name,event_id)` unique 중복 검사와 Outbox 기록을 같은 event DB 트랜잭션으로 처리한다. 중복 예외는 트랜잭션 프록시 밖인 `KafkaNotificationBinder`가 성공으로 종료한다. consumer 식별자는 `PriceAlertNotificationCreateCommand.CONSUMER_NAME`이 소유한다. `NotificationSaveEvent`는 `notificationId`와 `(notificationId,receiverId)` 자연 키 upsert로 Mongo 재전달을 흡수한다. Inbox를 Mongo 저장과 분산 트랜잭션처럼 취급하지 않는다.
