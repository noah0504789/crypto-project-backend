...
- 서비스 전체 CI(빌드+테스트+ArchUnit)는 **루트 task**로 실행: `./gradlew chatCi` · `userCi` · `marketCi` · `notificationCi` · `oauth2AuthorizationServerCi` · `oauth2ClientCi` · `websocketGatewayCi` · `apiGatewayCi` · `springCloudConfigCi` · `marketDetectionCi` · `upbitConnectorCi` · `outboxPollerCi` · `eurekaServerCi`. 전체는 `./gradlew serviceCi`.
- **변경 영향 서비스의 CI만 실행한다.** 여러 서비스에 걸친 변경은 필요한 `*Ci` task만 한 Gradle 호출에 묶는다. `serviceCi`는 공통 기반·빌드 로직·전체 런타임 설정처럼 13개 실행 서비스 모두에 영향이 있거나 사용자가 전체 CI를 명시한 경우에만 실행한다. 단순히 여러 모듈을 수정했다는 이유만으로 전체 CI를 선택하지 않는다.
