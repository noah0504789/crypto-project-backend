...
| **STOMP / WebSocket** | endpoint, destination과 wire payload | 기본 endpoint는 native `/ws-native`이고 SockJS `/ws-sockjs`는 선택 옵션이다. destination(`/msg/chat.send`, `/topic/chat/{roomId}`, `/queue/chat/ack`, `/queue/chat/badge`, `/queue/notification`)은 프론트와 `websocket-gateway/k6` 가 의존한다. 아웃바운드 payload 구조는 `docs/ARCHITECTURE.md §7.4`, 봉투 주의는 아래 |
