...
`common`(부모)은 16개 common-* 모듈을 `api`로 재수출하는 파사드다(`common/build.gradle`, `common-test`·`common-arch-test`·`common-actuator-*`는 제외). — **상세: `docs/modules/COMMON.md`**
...
|---|---|---|
| `market.v1`(MarketService.GetEnabledMarkets, PriceAlertSettingService.FindReceiverIds) | market-adapter-in | notification-adapter-out, upbit-connector, market 자체 |
...
- 토픽 카탈로그(`common-core/KafkaTopic`): `chatroom-event(.dlq)`, `chatroom-broadcast-event`, `chatmessage-event(.dlq)`, `chatmessage-broadcast-event`, `notification-event(.dlq)`, `web-notification-broadcast-event`, `market-broadcast-event`, `price-alert-detected-event`. `upbit-ticker-event`는 `upbit-connector`가 발행하고 market-detection Kafka Streams가 소비한다.
- Kafka Streams: `market-detection`의 `KafkaMarketDetectionBinder` + `PriceAlertDetectionProcessor`가 WindowStore `upbit-ticker-store`를 다루고, 계산은 `-application`의 `PriceChange`가 담당한다.
...
- 아웃바운드 wire payload(클라이언트가 실제 수신하는 형태, 계약):
  ...
  - 알림 → `convertAndSendToUser(receiverId, "/queue/notification", payload)`로 **사용자별** 전송, 클라이언트 구독 destination은 **`/user/queue/notification`**(user-destination). payload `StompWebNotificationPayload` `{ notificationId, type, title, body, createdAtMs(epoch millis, long), link, messageParts, data(PriceAlertPayload) }`. `notificationId`는 REST 읽음 처리와 STOMP 중복 제거에 쓴다. 근거 `StompWebNotificationAdapter` + `StompWebNotificationPayload`. 흐름: Kafka `web-notification-broadcast-event` consume(`WebNotificationBroadcastEventMapper`) → command → 위 전송. 로컬 세션 있는 대상에게만 push.
...
- CI task(루트 `build.gradle`): 서비스별 `chatCi`·`userCi`·`marketCi`·`notificationCi`·`oauth2AuthorizationServerCi`·`oauth2ClientCi`·`websocketGatewayCi`·`apiGatewayCi`·`springCloudConfigCi`·`marketDetectionCi`·`outboxPollerCi`·`eurekaServerCi`, 전체 집계 `serviceCi`. 각 CI가 `:common:common-arch-test:test`(ArchUnit)를 포함. `commonCi`/`protobufCi`는 없음.
