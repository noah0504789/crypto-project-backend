...
- 아웃바운드 wire payload(클라이언트가 실제 수신하는 형태, 계약):
  ...
  - 알림 → `convertAndSendToUser(receiverId, "/topic/notification/", payload)`로 **사용자별** 전송, 클라이언트 구독 destination은 **`/user/topic/notification/`**(user-destination). payload `StompWebNotificationPayload` `{ type, title, body, createdAtMs(epoch millis, long), link, data(Map<String,Object>) }`. 근거 `websocket-gateway-adapter-out/.../notification/adapter/out/stomp/StompWebNotificationAdapter.java` + `.../stomp/payload/StompWebNotificationPayload.java`. 흐름: Kafka `web-notification-broadcast-event` consume(`WebNotificationEventMapper`) → command → 위 전송. 로컬 세션 있는 대상에게만 push.
