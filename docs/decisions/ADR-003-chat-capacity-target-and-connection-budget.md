...
- **300명 목표는 배칭 없이 불가능하다.** 요구 90,000/s 대 능력 6,000/s다. **배칭은 2026-08-28 에 적용했다**(방별 100ms, [부하테스트 문서 §7-1](../../chat/load-test-results/chatmessage/websocket-gateway/2026-08-28/README.md)). STOMP wire payload 가 봉투(`{roomId, messages[]}`)가 되는 **외부 계약 변경**이었고 프론트를 함께 고쳤다. 실측 프레임 감소는 23배다.
