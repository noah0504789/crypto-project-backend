...
- `WebNotificationPayload`: `{ type, title, body, createdAtMs, link, messageParts, data }`. `data`는 `PriceAlertPayload` (`code`·`price`·`avgPrice`·`avgInterval`·`changeRate`·`threshold`·`occurredAtMs`)이며 STOMP `StompWebNotificationPayload.data`까지 같은 타입으로 이어진다. 소비 서비스와 프론트가 필드를 타입으로 읽는다.
- 알림 종류가 늘면 `data` 계약을 명시적으로 확장해야 한다. 현재 생성 경로는 `PRICE_ALERT` 하나다.
- `Notification.payload`(Mongo 저장용 `Map`)는 별개다. 이쪽은 `PriceAlertNotificationCreateCommand.toPayload()`가 `TypedPayload.toMap()`으로 만든다.
