...
- `[user]` / `[oauth2-as]` / `[oauth2-client]` = 해당 모듈 분석에서 확인/추가된 항목.
...
### 1.8 `[oauth2-client]` 로그아웃 시 JWT 미검증 파싱
