...
#### 5.4 부하테스트 재측정 — 완료 (2026-08-30)

**해소됐다.** 재측정을 마쳤고 보정 항목 다섯 중 넷을 반영했다.
결과는 [부하테스트 문서](chat/load-test-results/chatmessage/websocket-gateway/README.md).

| 보정 항목 | 상태 |
|---|---|
| `ACK_TIMEOUT_MS` 11초 | 적용 |
| `COLLECT_WINDOW_MS` 60초 | 적용 |
| VU별 자격증명 공급 | **적용** — 테스트 계정 300개, VU 당 1개 |
| 서버 메트릭 교차 검증 | 적용 — 거절·큐·GC 를 Prometheus·`jcmd` 로 대조 |
| DLQ 적체 · `stomp-in` 포화 원인 | **미측정으로 남는다** |

`stomp-in` 이 32/32 로 차던 것은 이번 측정에서 재현되지 않았다(큐 깊이 최대 0~2).
당시 SerialGC 정지가 원인이었을 가능성이 크지만 **확인하지 않았다.**
...
```
개발계   16GB 맥에 서비스·인프라·모니터링 26개 + 부하.  회차당 swapin 8~21GB
