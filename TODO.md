...
| 스레드 | 활성 최대 | 활용률 | 거절 | 큐 최대 | 미도달 | p90 |
|---:|---:|---:|---:|---:|---:|---:|
| 64 | 5 | 7.8% | 17,250 | 3,000 (포화) | 10,409 | 5,951ms |
| 16 | 8 | **50%** | **5,413** | 1,649 | **320** | **10,649ms** |

**스레드를 1/4 로 줄였더니 활용률이 6배 오르고 거절이 1/3 로 줄었다.** 락 경합 가설과 방향이 맞는다.
DB 압박도 함께 풀렸다(pending 75 → 8, 커넥션 타임아웃 8.2건 → 0건) — broker 가 덜 막히니 상류가 편해졌다.

다만 **소화 속도가 부족해 버려지던 것이 지연 도착으로 바뀌었다**(늦게 도착 0 → 81,544, 21.24%).
SLO 로 보면 수신률은 통과(99.92%)하나 p90 10.6초·ACK 78.68% 로 미달이다.

```
64  락 경합으로 5개만 가동 → 큐 포화 → 버림
16  8개 가동 → 안 버리지만 처리 속도 부족 → 지연
```

**최적점은 두 값 사이다.** 32 를 재서 곡선을 완성한다.
