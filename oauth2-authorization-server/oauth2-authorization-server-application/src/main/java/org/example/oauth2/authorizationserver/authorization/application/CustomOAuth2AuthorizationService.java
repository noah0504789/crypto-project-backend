...
public class CustomOAuth2AuthorizationService implements OAuth2AuthorizationService {
    ...
    public OAuth2Authorization findByToken(String token, OAuth2TokenType tokenType) {
        ...
        if (ACCESS_TOKEN.equals(tokenType)
                && accessTokenPort.existsByClaimKey(accessClaimKey)) {
            return getAccessOAuth2Authorization(token);
        }

        if (REFRESH_TOKEN.equals(tokenType)
                && refreshTokenPort.existsByEmailKey(refreshEmailKey)) {
    ...
    private OAuth2Authorization getAccessOAuth2Authorization(String token) {
        Map<String, String> claims = accessTokenPort.findClaims(token);
    ...
    private OAuth2Authorization getRefreshOAuth2Authorization(String token) {
        String email = refreshTokenPort.findEmail(oAuth2RegisteredClientProperties.registrationId(), token);
