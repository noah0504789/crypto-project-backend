...
- **best-effort push 인지**: 브로드캐스트 소비자는 DLQ/재시도가 없고, STOMP executor 큐가 포화하면 push 태스크를 버린다(`stomp.executor.rejected{pool, kind}` — `kind` 로 브로드캐스트·ACK·뱃지·알림을 갈라 읽는다. 합산하면 안 된다). **유실을 클라이언트가 감지해 재조회하는 경로는 없다** — 프론트는 재연결 시 재구독만 하고 wire payload에 방별 순번이 없어 갭 감지가 불가능하다(→ `../TODO.md` 5.5). "재조회로 복구된다"를 전제로 설계하지 않는다. durable로 바꾸려면 chat/notification 영속 경로와 함께 설계한다. 어느 거절이 무엇을 잃는지는 `../docs/SERVICE_FLOWS.md` §9 표를 본다.
...
- **발신자가 결과를 모르는 실패를 만들지 않는다**: 거절·검증 실패·서버 오류 모두 `clientMessageId` 를 담은 실패 ACK 로 돌려준다(→ `../docs/SERVICE_FLOWS.md` §9). 새 실패 경로를 만들 때 이 원칙부터 확인한다 — 현재 유일한 예외는 inbound 큐 거절이다(`../TODO.md` 5.14).
- **ACK·뱃지는 `brokerChannel` 을 지나지 않는다**: `DirectStompChatMessageAckAdapter`·`DirectStompMyChatRoomBadgeAdapter`가 `LocalSessionCache` 에서 세션과 **목적지별 구독 ID** 를 찾아 `clientOutboundChannel` 로 직접 넣는다. 구독 ID 가 빠지면 클라이언트가 프레임을 매칭하지 못해 **조용히 전달되지 않는다** — SUBSCRIBE/UNSUBSCRIBE·세션 제거 경로를 늘릴 때 구독 ID 정리도 함께 본다. 폴백은 두지 않는다. 뱃지 대상의 로컬 세션 부재는 정상이라 `chat.badge.direct.skipped`, 로컬 세션의 구독 부재·전송 오류는 `chat.badge.direct.failed` 로 분리한다. ACK 실패는 `chat.message.ack.direct.failed` 로 관측한다.
...
| [`...application/.../session/application/cache/LocalSessionCache.java`](websocket-gateway-application/src/main/java/org/example/websocket/gateway/session/application/cache/LocalSessionCache.java) | 로컬 세션·구독 인덱스(push 판정, ACK·뱃지 구독 ID). 상한 축출을 두지 않는다 — 축출이 정리 경로를 끊는다 |
...
| [`...adapter-out/.../chatroom/adapter/out/stomp/DirectStompMyChatRoomBadgeAdapter.java`](websocket-gateway-adapter-out/src/main/java/org/example/websocket/gateway/chatroom/adapter/out/stomp/DirectStompMyChatRoomBadgeAdapter.java) | 뱃지 직접 전송(`brokerChannel` 우회, 세션·구독 부재 시 실패 기록) |
| [`...adapter-out/.../chatmessage/adapter/out/stomp/DirectStompChatMessageAckAdapter.java`](websocket-gateway-adapter-out/src/main/java/org/example/websocket/gateway/chatmessage/adapter/out/stomp/DirectStompChatMessageAckAdapter.java) | ACK 직접 전송(`brokerChannel` 우회, 세션·구독 부재 시 실패 기록) |
| [`...adapter-out/.../websocket/adapter/out/stomp/DirectStompSessionSender.java`](websocket-gateway-adapter-out/src/main/java/org/example/websocket/gateway/websocket/adapter/out/stomp/DirectStompSessionSender.java) | 세션·구독 헤더 구성과 `clientOutboundChannel` 직접 전송 공통화 |
| [`...adapter-out/.../websocket/adapter/out/config/OutboundSchedulerConfig.java`](websocket-gateway-adapter-out/src/main/java/org/example/websocket/gateway/websocket/adapter/out/config/OutboundSchedulerConfig.java) | 메시지 배칭·뱃지 conflation 전용 scheduler와 종료 수명주기 |
