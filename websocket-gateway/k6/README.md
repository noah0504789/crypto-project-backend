...
현재 Gateway 계약(`/ws-sockjs`, `/ws-native`, `/msg/chat.send`, `/topic/chat/{roomId}`)에 맞춘 STOMP 부하테스트다.
...
그 측정 도구는 Kafka 영속 이벤트를 직접 발행해 입력 손실과 WebSocket·ACK·브로드캐스트 비용을 제외한다.
...
#288 이후 메시지 저장은 `chat_room_membership`을 만들지 않는다. 이 도구는 송신 권한에 필요한
