...
const room = db.chat_room.findOne({_id: ObjectId("'"$ROOM_ID"'")});
print("");
print("남길 것:");
...
print("  chat_message 삭제         " + db.chat_message.deleteMany({}).deletedCount);
print("  chat_room_membership 삭제 " + db.chat_room_membership.deleteMany({}).deletedCount);
db.chat_room.updateOne(
  {_id: roomId},
  {$set: {msg_cnt: 0, popularity: 0}, $unset: {memberIds: "", msgCnt: ""}}
);
print("  chat_room 카운터 초기화");
'
