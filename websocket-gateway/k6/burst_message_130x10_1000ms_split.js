...
const local_vus = Number(__ENV.LOCAL_VUS || 65);
const total_vus = Number(__ENV.TOTAL_VUS || 130);

const num_vus = local_vus;
const num_msgs_per_user = Number(__ENV.MSGS_PER_USER || 10);
  ...
  const res = ws.connect(url, {}, function (socket) {
    function start_sending() {
      ...
      socket.setInterval(function () {
        ...
        const content = `CID:${msg_id}|burst-message-${instance_name}-${vu}-${sent}`;
        const now = Date.now();
...
export function handleSummary(data) {
  ...
  const clean_result = `
========== clean_broadcast_result (${instance_name}) ==========
