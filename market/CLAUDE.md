...
- **캐시 무효화 흐름 보존**: 활성 마켓은 Caffeine `@Cacheable("markets", key 'enabled')`로 서빙하고, 카탈로그 변경은 `MarketCatalogChangedBroadcastEvent`(Outbox → `market-broadcast-event`)로 발행 → 각 인스턴스가 `marketCatalogChangedBroadcastEventConsumer`(group `market-broadcast-${app.instance-id}`, 인스턴스마다 고유)로 받아 `@CacheEvict`한다. 이 **per-instance 그룹**을 공유 그룹으로 바꾸면 일부 인스턴스만 evict되어 캐시 불일치가 난다 — 변경 금지(→ `../.claude/rules/external-contracts.md`). 캐시 이름은 `MarketCacheNames.MARKETS` 상수로만.
- **Outbox 발행 보존**: 카탈로그 쓰기는 `OutboxEventListPublishPort.publish`로 발행한다. 공용 `JpaOutbox`가 `catalog="event"`를 명시하므로 market datasource의 같은 connection으로 `market.*` 변경과 `event.outbox` 저장을 한 트랜잭션에 묶는다. market 계정에는 `event.outbox`의 `SELECT, INSERT` 권한이 필요하다. `MarketCatalogChangedBroadcastEvent`는 `getDomainType()=MARKET`, `getDispatchType()=BROADCAST`를 override한다 — 새 브로드캐스트 이벤트 추가 시 두 분류를 잊지 않는다.
...
- **idgen 설정 로드 유지**: `JpaPriceAlertSetting`이 `@SnowflakeId`를 쓰므로 `SnowflakeIdProvider`가 필요하다. 이 빈은 `@ConditionalOnProperty(idgen.epoch)`라 config.name에 `idgen`이 있어야 등록된다(`application.yml`의 `spring.cloud.config.name`에 `idgen` 포함). `idgen`을 빼면 ID 생성 빈이 없어 엔티티 저장이 깨진다(→ `../docs/modules/COMMON.md §6`).
