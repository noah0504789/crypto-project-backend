...
- **Outbox 발행 보존**: 카탈로그 쓰기는 `OutboxEventListPublishPort.publish`로 발행한다. `MarketCatalogChangedBroadcastEvent`는 `getDomainType()=MARKET`, `getDispatchType()=BROADCAST`를 override한다 — 새 브로드캐스트 이벤트 추가 시 두 분류를 잊지 않는다.
