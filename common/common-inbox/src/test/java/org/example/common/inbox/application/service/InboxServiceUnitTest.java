...
class InboxServiceUnitTest {
    ...
    @Test
    @DisplayName("event_id 선점을 저장소의 INSERT 메서드에 위임한다")
    void saveFlushesInbox() {
        sut.save("notification.price-alert", "event-1");

        verify(repository).insertAndFlush(any(Inbox.class));
    }

    @Test
    @DisplayName("동일 consumer와 event_id가 이미 처리됐으면 비즈니스 처리를 실행하지 않는다")
    void saveThrowsDuplicateInboxException() {
        doThrow(new DuplicateInboxException("notification.price-alert", "event-1", new IllegalStateException("duplicate")))
                .when(repository)
                .insertAndFlush(any(Inbox.class));
