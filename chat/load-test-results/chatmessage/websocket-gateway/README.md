...
회차를 측정하기 전까지 기록하지 않는다. 새 회차에서는 로컬 세션이 없는 방 멤버를 정상 skip으로
센 `chat.badge.direct.skipped`와, 로컬 세션의 구독 누락·outbound 전송 오류만 세는
`chat.badge.direct.failed`를 분리해 기록한다.
