...
class PriceAlertDetectionPropertiesUnitTest {
    ...
    void detectionDurationSettingsMustBeValid() {
        assertThat(validator.validate(properties(0, Duration.ofSeconds(10), minutes(3), minutes(3))))
                .extracting(violation -> violation.getPropertyPath().toString())
                .containsExactly("windowMinutes");

        assertThat(validator.validate(properties(3, Duration.ZERO, minutes(3), minutes(3))))
                .extracting(violation -> violation.getPropertyPath().toString())
